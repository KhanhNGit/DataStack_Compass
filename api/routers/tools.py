...
# =============================================================================
# 5. GET /api/v1/tools/search — Full-text search
#    (Đặt TRƯỚC /{tool_name} để FastAPI match path đúng)
# =============================================================================


@router.get(
    "/search",
    summary="Full-text search tools & versions",
    response_model=BaseResponse,
)
async def search_tools(
    q: str = Query(
        ..., min_length=1, max_length=200,
        description="Search query (tool name hoặc version)",
    ),
    db=Depends(get_db),
):
    """Tìm kiếm tool_name và version bằng LIKE '%query%'.

    StarRocks hỗ trợ LIKE pattern matching — trả về top 10 kết quả
    sorted by relevance (exact match trước, partial match sau).
    """
    like_pattern = f"%{q}%"
    ...
    sql = f"""
        SELECT
            ...
            g.eol_date,
            CASE
                WHEN g.tool_name = %s THEN 1
                WHEN g.tool_name LIKE %s THEN 2
                ELSE 3
            END AS relevance
        FROM {_GOLD_SUMMARY} g
        WHERE g.tool_name LIKE %s
        ...
        SELECT
            ...
            0 AS total_cve_critical,
            0 AS total_cve_high,
            NULL AS eol_date,
            CASE
                WHEN r.version = %s THEN 1
                WHEN r.version LIKE %s THEN 2
                ELSE 3
            END AS relevance
        FROM {_SILVER_RELEASES} r
        WHERE r.version LIKE %s

        ORDER BY relevance ASC, tool_name ASC
        LIMIT 10
    """

    with db.cursor() as cursor:
        cursor.execute(sql, (q, like_pattern, like_pattern, q, like_pattern, like_pattern))
        rows = cursor.fetchall()

    # Remove relevance column from output
    results = [{k: v for k, v in row.items() if k != "relevance"} for row in rows]

    return BaseResponse(
        data=results,
        meta={"query": q, "result_count": len(results)},
    )
